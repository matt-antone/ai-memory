...
SERVER_NAME="${OPENCLAW_MCP_SERVER_NAME:-ai-memory}"
...
local_path="${PWD}/.openclaw/openclaw.json"

scope="${AI_MEMORY_INSTALL_SCOPE:-}"
if [[ -z "$scope" ]]; then
  scope="$(choose_install_scope "OpenClaw")"
fi
...
case "$scope" in
  ...
  *)
    echo "Unsupported OpenClaw install scope: $scope" >&2
    exit 1
    ;;
esac
...
if [[ "$exists" == "1" ]]; then
  ...
  if [[ -z "$conflict_mode" ]]; then
    if [[ "$managed" == "1" ]]; then
      conflict_mode="$(choose_conflict_mode "OpenClaw" "$scope" "$config_path" "managed")"
    else
      conflict_mode="$(choose_conflict_mode "OpenClaw" "$scope" "$config_path" "unmanaged")"
    fi
  fi

  if [[ "$conflict_mode" == "overwrite" && "$managed" != "1" ]]; then
    confirm "Overwrite the unmanaged OpenClaw ai-memory entry at $config_path?" "N" || exit 0
  fi
...
echo "Wrote OpenClaw MCP config for '$SERVER_NAME' to:"
echo "  $config_path"
echo
if [[ "$scope" == "project/local" ]]; then
  echo "OpenClaw project-local config requires OPENCLAW_CONFIG_PATH to point at:"
  echo "  $config_path"
else
