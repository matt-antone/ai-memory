-- Clean baseline install for fresh projects.
-- Schema only: no seed data, no migration history rows.
-- Existing deployments should continue using the ordered files in supabase/migrations/.
...
create index if not exists memory_items_search_vector_idx on public.memory_items using gin (search_vector);
create index if not exists memory_items_created_at_idx on public.memory_items (created_at desc);
create index if not exists memory_items_archived_idx on public.memory_items (is_archived);

create table if not exists public.memory_embeddings (
  id text primary key,
  item_id text not null references public.memory_items(id) on delete cascade,
...
create index if not exists memory_embeddings_item_id_idx on public.memory_embeddings (item_id);

create table if not exists public.memory_edges (
  id text primary key,
  from_id text not null references public.memory_items(id) on delete cascade,
  to_id text not null references public.memory_items(id) on delete cascade,
  edge_type text not null,
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);

create index if not exists memory_edges_from_idx on public.memory_edges (from_id);
create index if not exists memory_edges_to_idx on public.memory_edges (to_id);

create table if not exists public.memory_events (
  id text primary key,
  item_id text references public.memory_items(id) on delete set null,
  event_type text not null,
  payload jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);

create index if not exists memory_events_item_id_idx on public.memory_events (item_id);

alter table public.memory_namespaces enable row level security;
alter table public.memory_namespaces force row level security;
...
alter table public.memory_edges force row level security;
alter table public.memory_events enable row level security;
alter table public.memory_events force row level security;

revoke all on table public.memory_namespaces from anon, authenticated;
...
revoke all on table public.memory_edges from anon, authenticated;
revoke all on table public.memory_events from anon, authenticated;

drop policy if exists "service_role_all_memory_namespaces" on public.memory_namespaces;
create policy "service_role_all_memory_namespaces"
on public.memory_namespaces
for all
to service_role
using (true)
with check (true);

drop policy if exists "service_role_all_memory_items" on public.memory_items;
...
drop policy if exists "service_role_all_memory_embeddings" on public.memory_embeddings;
...
drop policy if exists "service_role_all_memory_edges" on public.memory_edges;
...
drop policy if exists "service_role_all_memory_events" on public.memory_events;
...
create or replace function public.memory_namespace_matches(item_namespace jsonb, requested jsonb)
returns boolean
language sql
immutable
as $$
...
revoke all on function public.memory_namespace_matches(jsonb, jsonb) from public, anon, authenticated;

create or replace function public.memory_lexical_query(p_query text)
returns tsquery
language sql
immutable
as $$
...
revoke all on function public.memory_lexical_query(text) from public, anon, authenticated;
...
as $$
  with query_parts as (
    select public.memory_lexical_query(p_query) as lexical_query
  ),
  filtered as (
    select
      mi.*,
    ...
    left join public.memory_embeddings me on me.item_id = mi.id
    where mi.is_archived = false
      and public.memory_namespace_matches(mi.namespace, p_namespace)
      and (
...
revoke all on function public.memory_search(text, vector, jsonb, jsonb, text, integer) from public, anon, authenticated;

create or replace function public.memory_expand_context(
  p_item_ids text[],
...
as $$
  ...
  from walk
  join public.memory_items mi on mi.id = walk.item_id
  where mi.is_archived = false
    and not (mi.id = any(p_item_ids));
$$;

revoke all on function public.memory_expand_context(text[], integer) from public, anon, authenticated;
